...
export interface RealtimeServerEvents {
  ...
  "comment.created": (payload: { comment: EventComment; taskId: string; projectId: string; actorId: string }) => void;
  "presence.online": (payload: { userId: string; teamId?: string }) => void;
  "presence.offline": (payload: { userId: string; teamId?: string }) => void;
  "cursor:moved": (payload: { userId: string; projectId: string; x: number; y: number; view: string }) => void;
