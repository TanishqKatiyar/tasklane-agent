...
export default function ProjectPage({
  ...
  const updateTaskMutation = useMutation({
    ...
    onMutate: async ({ taskId, updates }) => {
      ...
      queryClient.setQueryData<Task[]>(
        ["tasks", params.projectId, filters],
  ...
  return (
    <div className="flex min-h-full flex-col">
          ...
          <div className="flex flex-shrink-0 items-center gap-3">
            <div className="flex -space-x-2">
              {MOCK_USERS.slice(0, 4).map((user) => (
                <div key={user.id} className="flex h-8 w-8 items-center justify-center rounded-full border-2 border-[var(--p-paper)] bg-[var(--p-paper-deep)] font-mono text-[10px] font-medium text-[var(--p-ink)]" title={user.name}>
            ...
            <button onClick={() => setShowCancelled(!showCancelled)}
              className={cn("flex h-9 items-center gap-1.5 rounded-lg border px-3 font-mono text-[10px] uppercase tracking-[0.18em] transition-colors",
                showCancelled
        ...
        <div className="flex gap-0.5">
          {VIEW_TABS.map(({ id, label, icon: Icon }) => (
            <button key={id} onClick={() => setActiveView(id)}
              className={cn("relative flex items-center gap-1.5 px-3 py-2 font-mono text-[11px] uppercase tracking-[0.16em] transition-colors",
                activeView === id ? "text-[var(--p-accent)]" : "text-[var(--p-ink-soft)] hover:text-[var(--p-ink)]")}>
              <Icon className="h-3.5 w-3.5" />
              {label}
              {activeView === id && <span className="absolute bottom-0 left-0 right-0 h-0.5 rounded-full bg-[var(--p-accent)]" />}
            </button>
          ))}
        </div>
        </div>
      ...
      {/* ── Filter Bar ── */}
      <FilterBar filters={filters} onFiltersChange={setFilters} members={MOCK_USERS} labels={MOCK_LABELS} />

      {/* ── Content ── */}
      <div className="flex-1 min-h-[640px]">
        {activeView === "board" && (
          <KanbanBoard tasks={filteredTasks} projectId={params.projectId} projectKey={project.key}
            isLoading={isLoading} filters={filters} showCancelled={showCancelled} onTaskClick={setSelectedTask} onTaskUpdate={handleTaskUpdate} />
        )}
        {activeView === "list" && (
          <ListView tasks={filteredTasks} projectKey={project.key}
            onTaskClick={setSelectedTask} onTaskUpdate={handleTaskUpdate} />
        )}
        {activeView === "calendar" && (
          <CalendarView tasks={filteredTasks} projectKey={project.key}
            onTaskClick={setSelectedTask} onTaskUpdate={handleTaskUpdate} />
        )}
        {activeView === "timeline" && (
      ...
      {/* ── Task Detail Dialog ── */}
      {selectedTask && (
        <TaskDetailDialog task={selectedTask} projectKey={project.key} open={!!selectedTask}
          onClose={() => setSelectedTask(null)} onUpdate={handleTaskUpdate} />
      )}
    </div>
  );
}
