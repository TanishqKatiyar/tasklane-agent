...
import { type ActivityItem, getTeamActivity } from '@/lib/notifications';
...
export default function TeamActivityPage({ params }: Props) {
    ...
    const onActivity = (payload: any) => {
      ...
      queryClient.setQueryData(queryKey, (old: any) => {
        ...
        return {
          ...old,
          pages: [
            { ...old.pages[0], data: [payload, ...(old.pages[0]?.data ?? [])] },
            ...old.pages.slice(1),
          ],
        };
      });
    };
    socket.on('activity.created' as any, onActivity);
    return () => {
      socket.off('activity.created' as any, onActivity);
    };
  }, [accessToken, teamId, queryClient]);
          ...
          {isLoading ? (
            ...
            <div className="flex flex-col items-center rounded-2xl border border-dashed border-[var(--p-rule)] bg-[var(--p-paper-deep)] px-6 py-16 text-center">
              ...
              <p className="mt-1 max-w-[280px] text-[13px] text-[var(--p-ink-soft)]">
                Activity will show here as the team works on tasks.
              </p>
            </div>
          ) : (
